    ...
    _app.getScript = function (url, options) {
        ...
        if (manifest)
            manifest.offline_resources.every(function (r) {
                var found = r.src === url.substring(2);
                if (found)
                    hash = '?h=' + r.hash;
                return !found;
            });
        ...
        executeCommand: function (args) {
            ...
            if (commandName && commandName.match(/^(Report|Export)/)) {
                ...
                if (serverActionErrorMessage) {
                    _touch.notify({ text: resourcesODP.UnableToExec + ' ' + serverActionErrorMessage, duration: 'long', force: true });
                    return;
                }
            }
    ...
    _app.AccountManager = {
        ...
        set: function (user) {
            ...
            var that = this,
                ...
                avatars,
                userData = { name: user.name, email: user.email, access_token: user.access_token, refresh_token: user.refresh_token, claims: user.claims };
            //if (!_host) {
