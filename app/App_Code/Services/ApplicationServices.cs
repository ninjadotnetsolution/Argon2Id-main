...
namespace Argon2Id.Services
{
    ...
    public partial class ApplicationServicesBase
    {
        ...
        {
            Argon2Id.Services.ApplicationServices.Initialize();
        }
        ...
        public virtual JObject UserSettings(Page p)
        {
            ...
            if (user != null)
            {
                ...
                EnumerateOfflineUserResources(pages, controllers);
                if ((controllers.Count > 0) && (appState == null))
                {
                    var odpSettings = ((JObject)(settings["odp"]));
                    if (odpSettings == null)
                    {
                        odpSettings = new JObject();
                        settings["odp"] = odpSettings;
                    }
                    var syncSettings = ((JObject)(odpSettings["sync"]));
                    if (syncSettings == null)
                    {
                        syncSettings = new JObject();
                        odpSettings["sync"] = syncSettings;
                    }
                    syncSettings["pages"] = JArray.FromObject(pages);
                    syncSettings["controllers"] = JArray.FromObject(controllers);
                }
            }
            return settings;
        }
        ...
        public static void RegisterStandardMembershipAccounts()
        {
            if (AuthorizationIsSupported)
            {
                ...
                if (Membership.GetUser("admin") == null)
                {
                    ...
                    admin = Membership.CreateUser("admin", "admin123%", "admin@Argon2Id.com", "ASP.NET", "Code OnTime", true, out status);
                    user = Membership.CreateUser("user", "user123%", "user@Argon2Id.com", "ASP.NET", "Code OnTime", true, out status);
