        ...
        protected virtual void CombineScripts(HttpContext context, bool caching, bool isSaaS, string culture)
        {
            ...
            if (isSaaS)
            {
                if (ApplicationServices.IsTouchClient)
                    ...
                    sb.AppendFormat(string.Format("$('<link></link>').appendTo($('head')).attr({{ href: '{0}/App_Themes/Argon2Id/_Theme_Aquarium.css?{0}', type: 'text/css', rel: 'stylesheet' }});", ApplicationServices.Version), baseUrl);
                try
                {
                ...
                sb.AppendFormat("var __targetFramework='4.7.2';__tf=4.0;__cothost='appfactory';__appInfo='Argon2Id|{0}';", BusinessRules.JavaScriptString(context.User.Identity.Name));
        ...
        protected virtual void BeginResponsePadding(HttpRequest request, StreamWriter sw)
        {
            ...
            else
            {
                if ((request.HttpMethod == "GET") && UriRestConfig.IsJSONPRequest(request))
                {
                    ...
                    sw.Write("Argon2Id=typeof Argon2Id=='undefined'?{{}}:Argon2Id;Argon2Id.{0}=", instance);
                }
            }
        }
